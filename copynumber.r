...
short <- paste0(opt$input,'/',opt$sample,'.hg38.50k.k50.nobad.varbin.short.txt')
...
out = paste0(opt$output,'/',opt$sample,'.hg38.50k.k50.varbin.data.copynumber.txt')
write.table(df, sep="\t", file=out, quote=F, row.names=F)

postscript(paste0(opt$sample,'.wg.cn.density.ps'), height=400, width=600)
